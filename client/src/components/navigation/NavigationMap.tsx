    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const hoveredCheckpoint = checkpoints.find((checkpoint) => {
        ...
        const distance = Math.sqrt(
          Math.pow(x - checkpointX, 2) + Math.pow(y - checkpointY, 2),
        );
      ...
      <CardContent className="p-2">
        <Tabs defaultValue="map" className="w-full">
          <TabsList className="bg-[#1B263B] mb-2">
            <TabsTrigger
              value="map"
              className="data-[state=active]:bg-[#415A77]"
            >
          ...
          <TabsContent value="weather" className="m-0">
            <div
              className="w-full bg-[#1B263B] rounded-md flex items-center justify-center border border-[#415A77]/20"
              style={{ height: "320px" }}
            >
